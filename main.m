...
vidInfo = imaqhwinfo(cam);                  %Se adquiere informaci�n del video
...
bulletsImshow16 = loadBullets('assets/enemy/bullets/bullet0', 3,16); %genera un arreglo de 16 balas con 3 imágenes cada una para la animación.
bulletsImshow5 = loadBullets('assets/enemy/bullets/type4/', 3,5); %genera un arreglo de 5 balas con 3 imágenes cada una para la animación.
hold off
...
set(howlImshow, 'AlphaData', howlAlpha); %asigna el canal alpha al personaje para poner transparencia al fondo de su imágen.
...
step(cam);              %Inicializa la c�mara para usarse
shot = step(cam);       %Toma un frame de video

masc1 = shot*0;         %m�scara superior, del tama�o del frame pero en color negro
masc2 = shot*0;         %m�scara derecha, del tama�o del frame pero en color negro
masc3 = shot*0;         %m�scara inferior, del tama�o del frame pero en color negro
masc4 = shot*0;         %m�scara izquierda, del tama�o del frame pero en color negro
[fil,col,cap] = size(shot);     %Obtiene las filas, columnas y capas del frame
masc1(1:fil*1/3,col*1/3:col*2/3,:) = 150;   %Crea la m�scara superior, asign�ndole color correspondiente a 150, diferenci�ndolo del negro de la imagen
masc2(fil*2/9:fil*7/9,col*3/4:col,:) = 150; %Crea la m�scara superior, asign�ndole color correspondiente a 150, diferenci�ndolo del negro de la imagen
masc3(fil*2/3:fil,col*1/3:col*2/3,:) = 150; %Crea la m�scara superior, asign�ndole color correspondiente a 150, diferenci�ndolo del negro de la imagen
masc4(fil*2/9:fil*7/9,1:col*1/4,:) = 150;   %Crea la m�scara superior, asign�ndole color correspondiente a 150, diferenci�ndolo del negro de la imagen
...
ind1 = find(masc1~=0);          %Encuentra los �ndices de la m�scara superior
ind2 = find(masc2~=0);          %Encuentra los �ndices de la m�scara derecha
ind3 = find(masc3~=0);          %Encuentra los �ndices de la m�scara inferior
ind4 = find(masc4~=0);          %Encuentra los �ndices de la m�scara izquierda

%Se crean auxiliares para las m�scaras
aux1 = masc1;
aux2 = masc2;
aux3 = masc3;
aux4 = masc4;

fondo = imresize(fondo, [fil,col]);         %Redimensiona el fondo al tama�o del frame

for i=1:1000        %Crea un ciclo de 1000 iteraciones. Este valor puede varias, ya que no se asign� tiempo al juego
     ...
     %Asigna la porci�n de frame correspondiente a la respectiva m�scara
     aux1(ind1) = snap(ind1);
     aux2(ind2) = snap(ind2);
     aux3(ind3) = snap(ind3);
     aux4(ind4) = snap(ind4);

     %Llama a la funci�n para sabes qu� m�scara detecta el color azul, el
     ...
     ifCollisionBullet = collision(howlImshow, bulletsImshow16);    %Llama a la funci�n para deterctar si el personaje colisiona con las balas

     if(ifCollisionBullet == 1)         %Analiza el valor de retorno de la colisi�n
        f = msgbox('has perdido!');     %Muestra una ventana con el mensaje de derrota
        break;                          %Termina el ciclo al perder el juego
     else
         %Analiza qu� m�scaras detectaron el color y mueve el personaje en la respectiva direcci�n
         if(blue1 == 1)
     ...
     snap(ind) = fondo(ind);        %Asigna las porciones de frame correspondientes a las m�scarasa la imagen de fondo
     step(hVideoIn,snap);           %Abre el player para reproducir video a partir de los frames
end
release(cam);                       %Libera el controlador de la c�mara
