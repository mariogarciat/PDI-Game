...
vidInfo = imaqhwinfo(cam);                  %Se adquiere información del video
...
step(cam);              %Inicializa la cámara para usarse
shot = step(cam);       %Toma un frame de video
     
masc1 = shot*0;         %máscara superior, del tamaño del frame pero en color negro
masc2 = shot*0;         %máscara derecha, del tamaño del frame pero en color negro
masc3 = shot*0;         %máscara inferior, del tamaño del frame pero en color negro
masc4 = shot*0;         %máscara izquierda, del tamaño del frame pero en color negro    
[fil,col,cap] = size(shot);     %Obtiene las filas, columnas y capas del frame
masc1(1:fil*1/3,col*1/3:col*2/3,:) = 150;   %Crea la máscara superior, asignándole color correspondiente a 150, diferenciándolo del negro de la imagen
masc2(fil*2/9:fil*7/9,col*3/4:col,:) = 150; %Crea la máscara superior, asignándole color correspondiente a 150, diferenciándolo del negro de la imagen
masc3(fil*2/3:fil,col*1/3:col*2/3,:) = 150; %Crea la máscara superior, asignándole color correspondiente a 150, diferenciándolo del negro de la imagen
masc4(fil*2/9:fil*7/9,1:col*1/4,:) = 150;   %Crea la máscara superior, asignándole color correspondiente a 150, diferenciándolo del negro de la imagen
...
ind1 = find(masc1~=0);          %Encuentra los índices de la máscara superior
ind2 = find(masc2~=0);          %Encuentra los índices de la máscara derecha
ind3 = find(masc3~=0);          %Encuentra los índices de la máscara inferior
ind4 = find(masc4~=0);          %Encuentra los índices de la máscara izquierda

%Se crean auxiliares para las máscaras
aux1 = masc1;           
aux2 = masc2;
aux3 = masc3;
aux4 = masc4;

fondo = imresize(fondo, [fil,col]);         %Redimensiona el fondo al tamaño del frame

for i=1:1000        %Crea un ciclo de 1000 iteraciones. Este valor puede varias, ya que no se asignó tiempo al juego
     ...
     positionsBullets = attack1(i, positionsBullets, howlImshow);
     
     %Asigna la porción de frame correspondiente a la respectiva máscara
     aux1(ind1) = snap(ind1);       
     aux2(ind2) = snap(ind2);
     aux3(ind3) = snap(ind3);
     aux4(ind4) = snap(ind4);
     
     %Llama a la función para sabes qué máscara detecta el color azul, el
     ...
     ifCollisionBullet = collision(howlImshow, bulletsImshow16);    %Llama a la función para deterctar si el personaje colisiona con las balas

     if(ifCollisionBullet == 1)         %Analiza el valor de retorno de la colisión
        f = msgbox('has perdido!');     %Muestra una ventana con el mensaje de derrota
        break;                          %Termina el ciclo al perder el juego
     else
         %Analiza qué máscaras detectaron el color y mueve el personaje en la respectiva dirección
         if(blue1 == 1)                                     
     ...
     snap(ind) = fondo(ind);        %Asigna las porciones de frame correspondientes a las máscarasa la imagen de fondo
     step(hVideoIn,snap);           %Abre el player para reproducir video a partir de los frames
end
release(cam);                       %Libera el controlador de la cámara
